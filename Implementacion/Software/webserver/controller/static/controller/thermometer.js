var Thermometer = (function(d3) {
	...
	Thermometer.prototype.render = function(container_id, currentTemp, minTemp, maxTemp, desiredTemp, title) {
		...
		this._svg = d3.select(container)
			...
			.attr("height", this._config.height + 50);

		_defineBulbGradient.call(this);
		_renderTube.call(this);
		_initAxis.call(this);
		// _renderExtremes.call(this);
		_renderDesired.call(this);
		_renderMercury.call(this);
		_renderAxis.call(this);
		
		this._svg.append("text")
			.attr("class", _createCssClass.call(this, "current", "label"))
			.attr("x", (this._config.width / 2))
			.attr("y", this._config.height + 25)
			.attr("text-anchor", "middle")
			// .style("font-family", "Varela Round")
			.style("font-size", "16px")
			.text(`${this._value.current} ºC`);
        
        this._svg.append("text")
			.attr("x", (this._config.width / 2))
			.attr("y", this._config.height + 50)
			.attr("text-anchor", "middle")
			// .style("font-family", "Varela Round")
			.style("font-size", "16px")
			.text(title);
	...
	Thermometer.prototype.setDesiredValue = function(value) {
		this._value.desired = value;
		_update.call(this);
	};
	...
	Thermometer.prototype.setCurrentValue = function(value) {
		...
		this._svg
			.select("." + _createCssClass.call(this, "current", "label"))
			.text(`${value} ºC`);
	};
	...
	function _updateDesiredMark() {
		this._svg
			.select("." + _createCssClass.call(this, "desired", "line"))
			.attr("y1", this._axisData.scale(this._value.desired))
			.attr("y2", this._axisData.scale(this._value.desired));

		// Desired temperature value
		this._svg
			.select("." + _createCssClass.call(this, "desired", "label"))
			.attr("y", this._axisData.scale(this._value.desired) - 4);
	}
	...
	function _renderDesired() {
		...
		this._svg
			...
			.attr("class", _createCssClass.call(this, label, "label"))
			.attr("x", this._config.width / 2 + this._config.tubeWidth / 2 + 2)
			.attr("y", this._axisData.scale(this._value.desired) + textOffset)
			.attr("dy", null)
			.text(label)
			.style("fill", textCol)
			.style("font-size", "10px");
