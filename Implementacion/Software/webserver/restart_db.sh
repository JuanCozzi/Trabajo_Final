...
echo "from django.contrib.auth import get_user_model; User = get_user_model(); superuser = User.objects.create_superuser('admin@admin.com', 'a'); from controller.models import Device, DeviceOutputForm; Device.objects.create(device_id=0, user=superuser); form = DeviceOutputForm({'device_id': 0, 'output': 1}); print('ERRORS ', form.errors.get_json_data()); form.save();" | python manage.py shell
